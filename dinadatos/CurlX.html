<?php
function Dead(string $text){ echo $text; }
...
class CurlX
{
    ...
    private static function SetCookie(string $file) : void 
    {
        ...
        if(!is_dir(self::$current_dir.'/Cache/')) mkdir(self::$current_dir.'/Cache/', 0755, true);
        // PHP7.4+
    ...
    public static function CaptchaV3(string $Data): string {
            $K = explode("&", $Data)[0];
            $R = self::Get("https://www.google.com/recaptcha/api2/anchor?ar=1&k=$Data");
            $Token = self::ParseString($R->body, '<input type="hidden" id="recaptcha-token" value="', '"');
            
            $NewData = ['c' => $Token, 'reason' => 'q', 'k' => $K];
            $R = self::Post("https://www.google.com/recaptcha/api2/reload?k=$K", $NewData);
            $Captcha = self::ParseString($R->body, '["rresp","', '"');
            return $Captcha;
    }
